...
class CodeBuilder:
    def __init__(self, classpath, clsname, class_name, class_data):
        ...
        self._included_constant_fields = set(
            class_data.query("implementation=='JAVA' and type=='static field'").index
        )
        self._included_fields = set(
            class_data.query(
                "implementation=='JAVA' and type=='dynamic variable'"
            ).index
        )
        self._included_methods = set(
            class_data.query(
                "implementation=='JAVA' and type in ['method', 'static method']"
            ).index
        )
        self._optional_java_methods = set(
            class_data.query("implementation=='JAVA' and type=='optional method'").index
        )
    ...
    def code_method(
        ...
        for py5_name, py5_decorator in zip(py5_names, py5_decorators):
            ...
            if static:
                first_param, classobj, moduleobj, decorator = (
                    "cls",
                    "cls._cls",
                    self._class_name,
                    "@classmethod",
            ...
            if len(method_data) == 1:
                ...
                if (
                    ref.PY5_SKIP_PARAM_TYPES.intersection(params)
                    or rettype in ref.PY5_SKIP_RETURN_TYPES
                ):
                    logger.warning(
    ...
    def code_extra_module(self, class_name, filename):
        ...
        method_code = "\n\n".join(
            cb.split("*** END METHODS ***")[0]
            for cb in code.split("*** BEGIN METHODS ***")[1:]
        )
        ...
        for (
            ...
            if decorator == "@overload":
                ...
                decorator == "@property"
            ):  # old way of coding properties before fake decorators
                self.module_members.append(
    ...
    def run_builder(self):
        for field_name, field_value in sorted(self._constant_field_data.items()):
            if field_name in self._included_constant_fields:
                ...
                field_name not in self._all_known_fields_and_methods
        ...
        for missing_constant_field_name in (
            self._included_constant_fields - self._constant_field_data.keys()
        ):
            logger.warning(
        ...
        for method_name, method_data in sorted(self._method_data.items()):
            if method_name in self._included_methods:
                ...
                method_name not in self._all_known_fields_and_methods
