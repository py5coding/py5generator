...
def doclink_to_title_map():
    ...
    for docfile in sorted(PY5_API_EN.glob("*.txt")):
        ...
        if (
            item_type in ["class", "pseudoclass"]
            or item_type in ["line magic", "cell magic"]
            or group in ["Sketch", "Py5Functions", "Py5Magics"]
        ):
            title = name
        elif group == "Py5Tools":
...
def prepare_mapping(method_signatures_lookup):
    ...
    for docfile in sorted(PY5_API_EN.glob("*.txt")):
        ...
        description = "\n".join(
            [textwrap.fill(d, 80) for d in description.splitlines()]
        )
        ...
        if item_type in ["line magic", "cell magic"]:
            ...
            if tuple_key not in method_signatures_lookup:
                ...
                for params, rettype in method_signatures_lookup[tuple_key]:
                    ...
                    for p in [
                        p.replace("*", "") for p in params if p and p not in ["/", "*"]
                    ]:
...
class TemplateMapping:
    ...
    def __getitem__(self, item):
        ...
        value = textwrap.indent(
            raw_value, prefix=(" " * TemplateMapping.INDENTING.get(kind, 0))
        ).strip()
        return value
