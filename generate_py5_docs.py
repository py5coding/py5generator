...
parser.add_argument(
    "dest_dir", action="store", help="location to write documentation files to"
)
parser.add_argument(
    "py5_doc_ref_dir",
    action="store",
        ...
        for s in signatures:
            ...
            if params:
                ...
                for i, (param, vardesc) in enumerate(new_params):
                    ...
                    new_params2.append(
                        f"{param}{maybe_comma}  # {vardesc}"
                        if vardesc
                        else f"{param}{maybe_comma}"
                    )
            ...
            new_signatures.append(
                black.format_str(
                    f"def {name}({params}){ret}: pass",
                    mode=black.Mode(line_length=line_length),
                )[4:-11]
            )

        out += "\n## Signatures\n\n```python\n"
        has_multi_line_signature = (
            max(len(s.strip().split("\n")) for s in new_signatures) > 1
        )
