...
try:
    for docfile in sorted(PY5_API_EN.glob('*.txt')):
        ...
        if doc.meta.get('pclass') != 'PShape' or doc.meta.get('name') not in ['begin_contour()', 'end_contour()', 'stroke_join()', 'stroke_cap()', 'stroke_weight()', 'no_fill()', 'fill()', 'no_stroke()', 'stroke()', 'bezier_vertex()', 'bezier_detail()', 'quadratic_vertex()', 'curve_detail()']:
            continue
